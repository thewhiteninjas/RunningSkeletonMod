...
public final class ElephantAnimationManager {
    ...
    private static final long ANIMATION_DURATION_MILLIS = 1800;
    ...
    private static final String TEXTURE_PATH_FORMAT =
            "assets/" + ElephantGreenScreenEffect.MOD_ID + "/textures/elephant/frame_(%d).png";
    private static final String SOUND_PATH =
            "assets/" + ElephantGreenScreenEffect.MOD_ID + "/sounds/elephant.ogg";
    ...
    public static boolean startAnimation() {
        ...
        if (!ensureTexturesLoaded()) {
            LOGGER.warn("[ElephantGreenScreenEffect] Cannot start animation: frame textures are missing or invalid. "
                    + "Check that all 48 files exist at "
                    + "src/main/resources/assets/" + ElephantGreenScreenEffect.MOD_ID
        ...
        Minecraft.getInstance().getSoundManager().play(
                SimpleSoundInstance.forUI(ModSounds.ELEPHANT_SOUND.get(), 1.0F));
        ...
        for (int i = 1; i <= TOTAL_FRAMES; i++) {
            ...
            try (InputStream stream = ElephantAnimationManager.class.getClassLoader().getResourceAsStream(path)) {
                ...
                ResourceLocation location = ResourceLocation.fromNamespaceAndPath(
                        ElephantGreenScreenEffect.MOD_ID, "dynamic/frame_" + i);
